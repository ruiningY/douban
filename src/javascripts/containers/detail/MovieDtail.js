...
class MovieDtail extends React.Component {
	...
	getmovieDetail( props){		
		...
		axios.get('../../../../public/json/hotmovie.json?id='+this.props.params.id)
		
		.then((res)=>{
		    //console.log(res);
		    that.setState({moviedetailhot:res.data})
...
componentWillMount(){
	...
	showMovieDetail(){
		var arr=[]
		 this.state.moviedetailhot.map((item,i)=>{	
